...
First, we need to determine pixel size in the upsampled domain. This is easy, if each camera pixel is $u \times u$ in size, the upsampled pixel size will be $\frac{u}{m} \times \frac{u}{m}$.

Next, given this size per pixel, we need the per-pixel units for the Fourier transform. Since the input image is discrete, we need to use the discrete Fourier transform (DFT) to calculate the frequency domain image. For the per-pixel units, \textbf{\textit{START HERE TODAY}}.
